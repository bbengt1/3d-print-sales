...
class Printer(Base):
    ...
    monitor_bed_target_c: Mapped[float | None] = mapped_column(Float, nullable=True)
    monitor_tool_target_c: Mapped[float | None] = mapped_column(Float, nullable=True)
    monitor_current_layer: Mapped[int | None] = mapped_column(Integer, nullable=True)
    monitor_total_layers: Mapped[int | None] = mapped_column(Integer, nullable=True)
    monitor_elapsed_seconds: Mapped[float | None] = mapped_column(Float, nullable=True)
    monitor_remaining_seconds: Mapped[float | None] = mapped_column(Float, nullable=True)
    monitor_eta_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    monitor_last_event_type: Mapped[str | None] = mapped_column(String(80), nullable=True)
    monitor_last_event_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    monitor_ws_connected: Mapped[bool | None] = mapped_column(Boolean, nullable=True)

...
@pytest.mark.asyncio
async def test_create_rate_validation(client):
    resp = await client.post(
        "/api/v1/rates",
...
@pytest.mark.asyncio
async def test_update_rate(client, seed_rates):
    rates = (await client.get("/api/v1/rates")).json()
    rate_id = rates[0]["id"]

    resp = await client.put(f"/api/v1/rates/{rate_id}", json={"value": 30})
    assert resp.status_code == 200
    assert float(resp.json()["value"]) == 30


@pytest.mark.asyncio
async def test_delete_rate(client, seed_rates):
    rates = (await client.get("/api/v1/rates")).json()
    rate_id = rates[0]["id"]

    resp = await client.delete(f"/api/v1/rates/{rate_id}")

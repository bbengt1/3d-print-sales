...
import { Moon, Sun, Printer, LogOut, Menu, X } from 'lucide-react';
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className="px-3 py-2 rounded-md text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent transition-colors no-underline"
              >
                {link.label}
              </Link>
            ))}
        ...
        <nav className="md:hidden border-t border-border px-4 py-2 bg-card">
          {navLinks.map((link) => (
            <Link
              ...
              className="block px-3 py-2 rounded-md text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-accent no-underline"
            >
              {link.label}
            </Link>
          ))}
